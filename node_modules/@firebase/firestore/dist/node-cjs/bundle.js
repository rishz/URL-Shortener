...
var LoadBundleTask = /** @class */ (function () {
    function LoadBundleTask() {
        ...
        this._taskCompletionResolver = new database21da52c3.Deferred();
        this._lastProgress = {
            taskState: 'Running',
            totalBytes: 0,
            totalDocuments: 0,
...
function loadBundle(db, bundleData) {
    ...
    database21da52c3.firestoreClientLoadBundle(database21da52c3.ensureFirestoreConfigured(db._delegate), db._databaseId, bundleData, resultTask);
    return resultTask;
}
function namedQuery(db, name) {
    return database21da52c3.firestoreClientGetNamedQuery(database21da52c3.ensureFirestoreConfigured(db._delegate), name).then(function (namedQuery) {
        if (!namedQuery) {
            return null;
        }
        return new database21da52c3.Query$1(db, new database21da52c3.Query(db._delegate, null, namedQuery.query));
    });
}
/**
 * @license
...
registerBundle(database21da52c3.Firestore);
